...
class DownBlock(nn.Module):
    def __init__(self, in_num_ch, out_num_ch):
        ...
        self.conv = nn.Sequential(
            nn.MaxPool2d(2), ConvDoubleBlock(in_num_ch, out_num_ch, 3)
        )
    ...
    def __init__(self, down_num_ch, up_num_ch, out_num_ch, upsample=True):
        ...
        if upsample == True:
            ...
            self.up = nn.ConvTranspose2d(
                up_num_ch, out_num_ch, 3, padding=1, stride=2
            )  # (H-1)*stride-2*padding+kernel_size
...
class Conv_BN_Act(nn.Module):
    def __init__(
        ...
        if is_bn:
            ...
            self.conv = nn.Conv2d(
                in_num_ch, out_num_ch, filter_size, stride, padding=padding
            )
        if activation == "lrelu":
...
class Act_Deconv_BN_Concat(nn.Module):
    def __init__(
        ...
        if upsample == True:
            ...
            self.up = nn.ConvTranspose2d(
                in_num_ch, out_num_ch, filter_size, padding=padding, stride=stride
            )  # (H-1)*stride-2*padding+kernel_size
...
class GANStandardGenerator(nn.Module):
    def __init__(
        ...
        self.down_8 = Conv_BN_Act(
            8 * first_num_ch, 8 * first_num_ch, activation="no"
        )  # 1 x 1
...
class GANShortGeneratorWithSpatialAttention(nn.Module):
    def __init__(
        ...
        self.att_4 = SpatialAttentionLayer(
            8 * first_num_ch, 8 * first_num_ch, 8 * first_num_ch, sample_factor
        )
        self.up_4 = Act_Deconv_BN_Concat(8 * first_num_ch, 8 * first_num_ch)
        self.att_3 = SpatialAttentionLayer(
            4 * first_num_ch, 16 * first_num_ch, 4 * first_num_ch, sample_factor
        )
        self.up_3 = Act_Deconv_BN_Concat(16 * first_num_ch, 4 * first_num_ch)
        self.att_2 = SpatialAttentionLayer(
            2 * first_num_ch, 8 * first_num_ch, 2 * first_num_ch, sample_factor
        )
        self.up_2 = Act_Deconv_BN_Concat(8 * first_num_ch, 2 * first_num_ch)
        self.att_1 = SpatialAttentionLayer(
            first_num_ch, 4 * first_num_ch, first_num_ch, sample_factor
        )
        self.up_1 = Act_Deconv_BN_Concat(4 * first_num_ch, first_num_ch)
        self.output = Act_Deconv_BN_Concat(2 * first_num_ch, out_num_ch, is_last=True)
...
class GANShortGeneratorWithSplitInputAndSpatialAttention(nn.Module):
    def __init__(
        ...
        self.down_1_1 = nn.Sequential(
            nn.Conv2d(1, first_num_ch, 4, 2, padding=1), nn.LeakyReLU(0.2, inplace=True)
        )
...
class GANShortGeneratorWithSymmetrySpatialAttention(nn.Module):
    def __init__(
        ...
        self.att_4 = SymmetrySpatialAttentionLayer(
            8 * first_num_ch, 8 * first_num_ch, 8 * first_num_ch, sample_factor
        )
        self.up_4 = Act_Deconv_BN_Concat(8 * first_num_ch, 8 * first_num_ch)
        self.att_3 = SymmetrySpatialAttentionLayer(
            4 * first_num_ch, 16 * first_num_ch, 4 * first_num_ch, sample_factor
        )
        self.up_3 = Act_Deconv_BN_Concat(16 * first_num_ch, 4 * first_num_ch)
        self.att_2 = SymmetrySpatialAttentionLayer(
            2 * first_num_ch, 8 * first_num_ch, 2 * first_num_ch, sample_factor
        )
        self.up_2 = Act_Deconv_BN_Concat(8 * first_num_ch, 2 * first_num_ch)
        self.att_1 = SymmetrySpatialAttentionLayer(
            first_num_ch, 4 * first_num_ch, first_num_ch, sample_factor
        )
        self.up_1 = Act_Deconv_BN_Concat(4 * first_num_ch, first_num_ch)
        self.output = Act_Deconv_BN_Concat(2 * first_num_ch, out_num_ch, is_last=True)
...
class GANShortGeneratorWithSymmetryResidualSpatialAttention(nn.Module):
    def __init__(
        ...
        self.att_4 = SymmetryResidualSpatialAttentionLayer(
            8 * first_num_ch, 8 * first_num_ch, 8 * first_num_ch, sample_factor
        )
        self.up_4 = Act_Deconv_BN_Concat(8 * first_num_ch, 8 * first_num_ch)
        self.att_3 = SymmetryResidualSpatialAttentionLayer(
            4 * first_num_ch, 16 * first_num_ch, 4 * first_num_ch, sample_factor
        )
        self.up_3 = Act_Deconv_BN_Concat(16 * first_num_ch, 4 * first_num_ch)
        self.att_2 = SymmetryResidualSpatialAttentionLayer(
            2 * first_num_ch, 8 * first_num_ch, 2 * first_num_ch, sample_factor
        )
        self.up_2 = Act_Deconv_BN_Concat(8 * first_num_ch, 2 * first_num_ch)
        self.att_1 = SymmetryResidualSpatialAttentionLayer(
            first_num_ch, 4 * first_num_ch, first_num_ch, sample_factor
        )
        self.up_1 = Act_Deconv_BN_Concat(4 * first_num_ch, first_num_ch)
        self.output = Act_Deconv_BN_Concat(2 * first_num_ch, out_num_ch, is_last=True)
...
class GANShortGeneratorWithSymmetryGateResidualSpatialAttention(nn.Module):
    def __init__(
        ...
        super(
            GANShortGeneratorWithSymmetryGateResidualSpatialAttention, self
        ).__init__()
        self.down_1 = nn.Sequential(
            nn.Conv2d(in_num_ch, first_num_ch, 4, 2, padding=1),
            nn.LeakyReLU(0.2, inplace=True),
        ...
        self.att_4 = SymmetryGateResidualSpatialAttentionLayer(
            8 * first_num_ch, 8 * first_num_ch, 8 * first_num_ch, sample_factor
        )
        self.up_4 = Act_Deconv_BN_Concat(8 * first_num_ch, 8 * first_num_ch)
        self.att_3 = SymmetryGateResidualSpatialAttentionLayer(
            4 * first_num_ch, 16 * first_num_ch, 4 * first_num_ch, sample_factor
        )
        self.up_3 = Act_Deconv_BN_Concat(16 * first_num_ch, 4 * first_num_ch)
        self.att_2 = SymmetryGateResidualSpatialAttentionLayer(
            2 * first_num_ch, 8 * first_num_ch, 2 * first_num_ch, sample_factor
        )
        self.up_2 = Act_Deconv_BN_Concat(8 * first_num_ch, 2 * first_num_ch)
        self.att_1 = SymmetryGateResidualSpatialAttentionLayer(
            first_num_ch, 4 * first_num_ch, first_num_ch, sample_factor
        )
        self.up_1 = Act_Deconv_BN_Concat(4 * first_num_ch, first_num_ch)
        self.output = Act_Deconv_BN_Concat(2 * first_num_ch, out_num_ch, is_last=True)
...
class GANShortGeneratorWithSplitInputAndSymmetryGateResidualSpatialAttention(nn.Module):
    def __init__(
        ...
        super(
            GANShortGeneratorWithSplitInputAndSymmetryGateResidualSpatialAttention, self
        ).__init__()
        self.down_1_1 = nn.Sequential(
            nn.Conv2d(1, first_num_ch, 4, 2, padding=1), nn.LeakyReLU(0.2, inplace=True)
        )
...
class GANShortGeneratorWithSplitInputChannelAttentionOneAndSpatialAttention(nn.Module):
    def __init__(
        ...
        super(
            GANShortGeneratorWithSplitInputChannelAttentionOneAndSpatialAttention, self
        ).__init__()
...
class GANShortGeneratorWithSplitInputChannelAttentionAllAndSpatialAttention(nn.Module):
    def __init__(
        ...
        super(
            GANShortGeneratorWithSplitInputChannelAttentionAllAndSpatialAttention, self
        ).__init__()
        ...
        self.att_4_s = SymmetryGateResidualSpatialAttentionLayer(
            8 * first_num_ch, 8 * first_num_ch, 8 * first_num_ch, sample_factor
        )
        self.up_4 = Act_Deconv_BN_Concat(8 * first_num_ch, 8 * first_num_ch)

        self.att_3_c = ChannelAttentionLayer(4 * first_num_ch, 4)
        self.att_3_s = SymmetryGateResidualSpatialAttentionLayer(
            4 * first_num_ch, 16 * first_num_ch, 4 * first_num_ch, sample_factor
        )
        self.up_3 = Act_Deconv_BN_Concat(16 * first_num_ch, 4 * first_num_ch)

        self.att_2_c = ChannelAttentionLayer(2 * first_num_ch, 2)
        self.att_2_s = SymmetryGateResidualSpatialAttentionLayer(
            2 * first_num_ch, 8 * first_num_ch, 2 * first_num_ch, sample_factor
        )
        self.up_2 = Act_Deconv_BN_Concat(8 * first_num_ch, 2 * first_num_ch)

        self.att_1_c = ChannelAttentionLayer(first_num_ch, 1)
        self.att_1_s = SymmetryGateResidualSpatialAttentionLayer(
            first_num_ch, 4 * first_num_ch, first_num_ch, sample_factor
        )
        self.up_1 = Act_Deconv_BN_Concat(4 * first_num_ch, first_num_ch)
        self.output = Act_Deconv_BN_Concat(2 * first_num_ch, out_num_ch, is_last=True)
...
class GANShortGeneratorWithChannelAttentionAllAndSymmetrySpatialAttention(nn.Module):
    def __init__(
        ...
        super(
            GANShortGeneratorWithChannelAttentionAllAndSymmetrySpatialAttention, self
        ).__init__()
...
class GANShortGeneratorWithChannelAttentionAllAndSpatialAttention(nn.Module):
    def __init__(
        ...
        super(
            GANShortGeneratorWithChannelAttentionAllAndSpatialAttention, self
        ).__init__()
        ...
        self.att_4_c = ChannelAttentionLayer(8 * first_num_ch, 8)
        self.att_4_s = SpatialAttentionLayer(
            8 * first_num_ch, 8 * first_num_ch, 8 * first_num_ch, sample_factor
        )
        self.up_4 = Act_Deconv_BN_Concat(8 * first_num_ch, 8 * first_num_ch)

        self.att_3_c = ChannelAttentionLayer(4 * first_num_ch, 4)
        self.att_3_s = SpatialAttentionLayer(
            4 * first_num_ch, 16 * first_num_ch, 4 * first_num_ch, sample_factor
        )
        self.up_3 = Act_Deconv_BN_Concat(16 * first_num_ch, 4 * first_num_ch)

        self.att_2_c = ChannelAttentionLayer(2 * first_num_ch, 2)
        self.att_2_s = SpatialAttentionLayer(
            2 * first_num_ch, 8 * first_num_ch, 2 * first_num_ch, sample_factor
        )
        self.up_2 = Act_Deconv_BN_Concat(8 * first_num_ch, 2 * first_num_ch)

        self.att_1_c = ChannelAttentionLayer(first_num_ch, 1)
        self.att_1_s = SpatialAttentionLayer(
            first_num_ch, 4 * first_num_ch, first_num_ch, sample_factor
        )
        self.up_1 = Act_Deconv_BN_Concat(4 * first_num_ch, first_num_ch)
        self.output = Act_Deconv_BN_Concat(2 * first_num_ch, out_num_ch, is_last=True)
...
class GANShortGeneratorWithSplitInputChannelAttentionAllAndSpatialAttentionNoBN(
    nn.Module
):
    def __init__(
        self,
        ...
        super(
            GANShortGeneratorWithSplitInputChannelAttentionAllAndSpatialAttentionNoBN,
            self,
        ).__init__()
        ...
        self.down_5 = Conv_BN_Act(
            8 * first_num_ch, 8 * first_num_ch, activation="no", is_bn=False
        )  # 8 x 8
        ...
        self.up_4 = Act_Deconv_BN_Concat(
            8 * first_num_ch, 8 * first_num_ch, is_bn=False
        )
        ...
        self.up_3 = Act_Deconv_BN_Concat(
            16 * first_num_ch, 4 * first_num_ch, is_bn=False
        )
        ...
        self.up_2 = Act_Deconv_BN_Concat(
            8 * first_num_ch, 2 * first_num_ch, is_bn=False
        )

        self.att_1_c = ChannelAttentionLayer(first_num_ch, 1)
        self.att_1_s = SymmetryGateResidualSpatialAttentionLayer(
            first_num_ch, 4 * first_num_ch, first_num_ch, sample_factor, is_bn=False
        )
        self.up_1 = Act_Deconv_BN_Concat(4 * first_num_ch, first_num_ch, is_bn=False)
        self.output = Act_Deconv_BN_Concat(
            2 * first_num_ch, out_num_ch, is_last=True, is_bn=False
        )
...
class SpatialAttentionLayer(nn.Module):
    def __init__(self, in_num_ch, gate_num_ch, inter_num_ch, sample_factor=(2, 2)):
        ...
        self.W_x = nn.Conv2d(
            in_num_ch, inter_num_ch, sample_factor, sample_factor, bias=False
        )
        self.W_g = nn.Conv2d(gate_num_ch, inter_num_ch, 1, 1)
        self.W_psi = nn.Conv2d(inter_num_ch, 1, 1, 1)
        self.W_out = nn.Sequential(
            nn.Conv2d(in_num_ch, in_num_ch, 1, 1), nn.BatchNorm2d(in_num_ch)
        )
...
class SymmetryGateResidualSpatialAttentionLayer(nn.Module):
    # only g
    def __init__(
        self, in_num_ch, gate_num_ch, inter_num_ch, sample_factor=(2, 2), is_bn=True
    ):
...
class MultiAttentionLayer(nn.Module):
    def __init__(
        ...
        in_num_ch,
        gate_num_ch,
        sample_factor_spatial=(2, 2),
        sample_factor_channel=16,
        kernel_stride_ratio=4,
        is_bn=True,
    ):
        ...
        self.AvgPool = nn.AvgPool2d(
            kernel_size=tuple([z * kernel_stride_ratio for z in sample_factor_spatial]),
            stride=sample_factor_spatial,
        )
...
class VAEBranch(nn.Module):
    def __init__(self, input_shape, init_channels, out_channels, squeeze_channels=None):
        ...
        self.hidden_conv = nn.Sequential(
            nn.GroupNorm(8, init_channels * 8),
            nn.ReLU(inplace=True),
            nn.Conv3d(
                init_channels * 8, self.squeeze_channels, (3, 3, 3), padding=(1, 1, 1)
            ),
        ...
        self.logvar_fc = nn.Linear(
            self.squeeze_channels // 2, self.squeeze_channels // 2
        )

        recon_shape = np.prod(self.input_shape) // (16 ** 3)

        self.reconstraction = nn.Sequential(
            nn.Linear(self.squeeze_channels // 2, init_channels * 8 * recon_shape),
            nn.ReLU(inplace=True),
        )

        self.vconv4 = nn.Sequential(
            nn.Conv3d(init_channels * 8, init_channels * 8, (1, 1, 1)),
            nn.Upsample(scale_factor=2),
        )

        self.vconv3 = nn.Sequential(
            nn.Conv3d(
                init_channels * 8, init_channels * 4, (3, 3, 3), padding=(1, 1, 1)
            ),
            nn.Upsample(scale_factor=2),
        ...
        self.vconv2 = nn.Sequential(
            nn.Conv3d(
                init_channels * 4, init_channels * 2, (3, 3, 3), padding=(1, 1, 1)
            ),
            nn.Upsample(scale_factor=2),
        ...
        self.vconv1 = nn.Sequential(
            nn.Conv3d(init_channels * 2, init_channels, (3, 3, 3), padding=(1, 1, 1)),
            nn.Upsample(scale_factor=2),
...
class UNet3D(nn.Module):
    ...
    def __init__(
        self, input_shape, in_channels=4, out_channels=3, init_channels=32, p=0.2
    ):
    ...
    def make_encoder(self):
        ...
        self.conv1a = nn.Conv3d(
            self.in_channels, init_channels, (3, 3, 3), padding=(1, 1, 1)
        )
        ...
        self.ds1 = nn.Conv3d(
            ...
            stride=(2, 2, 2),
            padding=(1, 1, 1),
        )  # down sampling and add channels
...
class NVNet3D(nn.Module):
    def __init__(
        self, input_shape, in_channels=4, out_channels=3, init_channels=16, p=0.2
    ):
        ...
        self.vae_branch = VAEBranch(
            input_shape, init_channels, out_channels=in_channels
        )
...
class CondConv2d(_ConvNd):
    def __init__(
        ...
        self._routing_fn = _routing(
            embeddings, num_experts
        )  # embeddings=shape of inputs_type label
    ...
    def forward(self, inputs, inputs_type):
        ...
        kernels = torch.sum(
            routing_weights[:, :, None, None, None, None] * self.weight, 1
        )
        for i in range(bs):
...
class AnatomyEncoderEncNew(nn.Module):
    def __init__(self, in_num_ch=7, first_num_ch=32, is_cond=False):
        ...
        self.down_3 = Conv_BN_Act_New(
            2 * first_num_ch, 4 * first_num_ch, is_cond=is_cond
        )
        self.down_4 = Conv_BN_Act_New(
            4 * first_num_ch, 8 * first_num_ch, is_cond=is_cond
        )
        self.down_5 = Conv_BN_Act_New(
            8 * first_num_ch, 8 * first_num_ch, activation="no", is_cond=is_cond
        )  # 8 x 8
...
class AnatomyEncoderDecNew(nn.Module):
    def __init__(
        self, first_num_ch=32, out_num_ch=8, output_act="softmax", is_cond=False
    ):
        super(AnatomyEncoderDecNew, self).__init__()
        # decoder part of anatomy encoder (unet)
        self.up_4 = Act_Deconv_BN_Concat_New(
            8 * first_num_ch, 8 * first_num_ch, is_cond=is_cond
        )
        self.up_3 = Act_Deconv_BN_Concat_New(
            16 * first_num_ch, 4 * first_num_ch, is_cond=is_cond
        )
        self.up_2 = Act_Deconv_BN_Concat_New(
            8 * first_num_ch, 2 * first_num_ch, is_cond=is_cond
        )
        self.up_1 = Act_Deconv_BN_Concat_New(
            4 * first_num_ch, first_num_ch, is_cond=is_cond
        )
        self.output = Act_Deconv_BN_Concat_New(
            2 * first_num_ch, out_num_ch, is_last=True, is_cond=is_cond
        )
        # if output_act == 'softmax':
        #     self.output_act = nn.Softmax()
        # else:
...
class ModalityEncoderNew(nn.Module):
    def __init__(
        self, img_num_ch=7, s_num_ch=8, first_num_ch=16, z_size=16, is_cond=False
    ):
...
class SPADEBlockNew(nn.Module):
    def __init__(
        self, input_size, in_num_ch=128, out_num_ch=128, s_num_ch=8, is_cond=False
    ):
...
class SPADE(nn.Module):
    def __init__(
        self, image_size=(192, 160), in_num_ch=7, z_size=16, z_num_ch=128, s_num_ch=8
    ):
        super(SPADE, self).__init__()
        self.z_num_ch = z_num_ch
        self.image_size = image_size
        self.zi_scaler = nn.Linear(
            z_size, image_size[0] * image_size[1] * z_num_ch // 1024
        )
        self.sp1 = SPADEBlock(
        ...
        self.sp2 = SPADEBlock(
            input_size=(image_size[0] // 16, image_size[1] // 16),
            in_num_ch=z_num_ch,
            out_num_ch=z_num_ch,
            s_num_ch=s_num_ch,
        )
        ...
        self.sp3 = SPADEBlock(
            input_size=(image_size[0] // 8, image_size[1] // 8),
            in_num_ch=z_num_ch,
            out_num_ch=z_num_ch,
            s_num_ch=s_num_ch,
        )
        ...
        self.sp4 = SPADEBlock(
            input_size=(image_size[0] // 4, image_size[1] // 4),
            in_num_ch=z_num_ch,
            out_num_ch=z_num_ch // 2,
            s_num_ch=s_num_ch,
        )
        self.up4 = nn.Upsample(scale_factor=(2, 2), mode="bilinear")
        self.sp5 = SPADEBlock(
            input_size=(image_size[0] // 2, image_size[1] // 2),
            in_num_ch=z_num_ch // 2,
            out_num_ch=z_num_ch // 4,
            s_num_ch=s_num_ch,
        )
        self.up5 = nn.Upsample(scale_factor=(2, 2), mode="bilinear")
        self.sp6 = SPADEBlock(
            input_size=(image_size[0], image_size[1]),
            in_num_ch=z_num_ch // 4,
            out_num_ch=z_num_ch // 8,
            s_num_ch=s_num_ch,
        )
        self.out = nn.Sequential(
            nn.Conv2d(z_num_ch // 8, in_num_ch, 1, 1), nn.Softplus()
        )

    def forward(self, si, zi):
        # pdb.set_trace()
        zi_rescale = self.zi_scaler(zi)
        zi_reshape = zi_rescale.reshape(
            -1, self.z_num_ch, self.image_size[0] // 32, self.image_size[1] // 32
        )
...
class GANShortGeneratorNew(nn.Module):
    def __init__(
        ...
        self.down_1 = conv2d(
            in_num_ch, first_num_ch, 4, 2, padding=1, embeddings=z_size
        )
        self.down_1_act = nn.LeakyReLU(0.2, inplace=True)
        self.down_2 = Conv_BN_Act_New2(
            first_num_ch, 2 * first_num_ch, is_cond=is_cond, embeddings=z_size
        )
        self.down_3 = Conv_BN_Act_New2(
            2 * first_num_ch, 4 * first_num_ch, is_cond=is_cond, embeddings=z_size
        )
        self.down_4 = Conv_BN_Act_New2(
            4 * first_num_ch, 8 * first_num_ch, is_cond=is_cond, embeddings=z_size
        )
        self.down_5 = Conv_BN_Act_New2(
            8 * first_num_ch,
            8 * first_num_ch,
            activation="no",
            is_cond=is_cond,
        ...
        self.up_4 = Act_Deconv_BN_Concat_New2(
            8 * first_num_ch, 8 * first_num_ch, is_cond=is_cond, embeddings=z_size
        )
        self.up_3 = Act_Deconv_BN_Concat_New2(
            16 * first_num_ch, 4 * first_num_ch, is_cond=is_cond, embeddings=z_size
        )
        self.up_2 = Act_Deconv_BN_Concat_New2(
            8 * first_num_ch, 2 * first_num_ch, is_cond=is_cond, embeddings=z_size
        )
        self.up_1 = Act_Deconv_BN_Concat_New2(
            4 * first_num_ch, first_num_ch, is_cond=is_cond, embeddings=z_size
        )
        self.output = Act_Deconv_BN_Concat_New2(
            2 * first_num_ch,
            out_num_ch,
            is_last=True,
            is_cond=is_cond,
...
class Discriminator(nn.Module):
    def __init__(
        self, in_num_ch=8, inter_num_ch=16, input_shape=[160, 192], is_patch_gan=False
    ):
        ...
        if is_patch_gan:
            ...
            self.fc = nn.Sequential(
                nn.Flatten(),
                nn.Linear(
                    int(input_shape[0] * input_shape[1] * 4 * inter_num_ch / (32 * 32)),
                    inter_num_ch * 16,
                ),
                nn.LeakyReLU(0.2),
...
class MultimodalModel(nn.Module):
    def __init__(
        ...
        z_sim_method="cosine",
        is_cond=True,
        input_output_act="softplus",
        target_output_act="softplus",
        target_model_name="U",
        fuse_method="mean",
        device=torch.device("cuda:0"),
        others={"mod_enc_s": True, "ana_dec_act": "softmax"},
    ):
        ...
        self.others = others
        (
            self.anatomy_encoder_enc_list,
            self.anatomy_encoder_dec,
        ) = self.define_anatomy_encoder_list(out_num_ch=s_num_ch)
        if self.others["old"]:
            self.modality_encoder = ModalityEncoder(
                img_num_ch=in_num_ch, s_num_ch=s_num_ch, first_num_ch=16, z_size=z_size
            ).to(self.device)
            self.input_decoder = SPADE(
            ...
            self.modality_encoder_list = self.define_modality_encoder_list(
                in_num_ch=in_num_ch, s_num_ch=s_num_ch, z_size=z_size
            )
            self.input_decoder_list = self.define_input_decoder_list(
                input_size=input_size,
                in_num_ch=in_num_ch,
                z_size=z_size,
                s_num_ch=s_num_ch,
                output_activation=input_output_act,
            )
        ...
        if target_model_name == "U":
            self.output_decoder = GANShortGenerator(
                in_num_ch=fuse_num_ch * s_num_ch,
                out_num_ch=out_num_ch,
                first_num_ch=64,
                input_size=input_size,
                output_activation=target_output_act,
            ).to(self.device)
        elif target_model_name == "U+SA":
            self.output_decoder = GANShortGeneratorWithSpatialAttention(
                in_num_ch=fuse_num_ch * s_num_ch,
                out_num_ch=out_num_ch,
                first_num_ch=64,
                input_size=input_size,
            ...
            self.output_decoder = GANShortGeneratorWithChannelAttentionAllAndSpatialAttention(
                in_num_ch=fuse_num_ch * s_num_ch,
                out_num_ch=out_num_ch,
                first_num_ch=64,
                input_size=input_size,
            ...
            self.output_decoder = GANShortGeneratorWithChannelAttentionAllAndSymmetrySpatialAttention(
                in_num_ch=fuse_num_ch * s_num_ch,
                out_num_ch=out_num_ch,
                first_num_ch=64,
                input_size=input_size,
        ...
        if is_discrim_s:
            self.discrim_s = Discriminator(in_num_ch=s_num_ch, inter_num_ch=16).to(
                self.device
            )
        if is_distri_z:
            self.distri_z = ModalityDistribution(z_size=z_size, inter_num_ch=128).to(
                self.device
            )
    ...
    def define_anatomy_encoder_list(self, out_num_ch=8, first_num_ch=32):
        ...
        if self.shared_ana_enc:
            encoder_list.append(
                AnatomyEncoderEncNew(
                    in_num_ch=self.in_num_ch,
                    first_num_ch=first_num_ch,
        ...
        decoder = AnatomyEncoderDecNew(
            first_num_ch=first_num_ch, out_num_ch=out_num_ch, is_cond=self.is_cond
        ).to(self.device)
    ...
    def compute_anatomy_encoding(self, inputs_list, mask_img):
        si_list = []
        for i in range(self.modality_num):
            inputs_type = (1 + i) * torch.ones(
                inputs_list[0].shape[0], 1, device=self.device
            )
            # inputs_type = i * torch.ones(inputs_list[0].shape[0], 1, device=self.device)
            if self.shared_ana_enc:
                feat_list = self.anatomy_encoder_enc_list[0](
                    inputs_list[i], inputs_type
                )
            else:
                feat_list = self.anatomy_encoder_enc_list[i](
                    inputs_list[i], inputs_type
                )
            si, _ = self.anatomy_encoder_dec(feat_list, inputs_type)
            if (
                "ana_dec_act" in self.others
                and self.others["ana_dec_act"] == "softplus"
            ):
                si_act = F.softplus(si)
            else:
                # pdb.set_trace()
                if (
                    "softmax_remove_mask" in self.others
                    and self.others["softmax_remove_mask"] == True
                ):
    ...
    def sample(self, z_mean, z_log_var):
        eps = torch.normal(0, 1, size=(z_mean.shape[0], z_mean.shape[1])).to(
            self.device
        )
    ...
    def compute_modality_encoding(self, inputs_list, si_list, phase="train"):
        ...
        for i in range(self.modality_num):
            ...
            if self.others["old"]:
                ...
                z_mean, z_log_var = self.modality_encoder_list[0](
                    inputs_list[i], si_list[i], inputs_type
                )
            else:
                z_mean, z_log_var = self.modality_encoder_list[i](
                    inputs_list[i], si_list[i], inputs_type
                )
    ...
    def reconstruct_input_si_zi(self, si_list, zi_list):
        ...
        for i in range(self.modality_num):
            inputs_type = (1 + i) * torch.ones(
                si_list[0].shape[0], 1, device=self.device
            )
            ...
            if self.others["old"]:
                ...
                xi_fake = self.input_decoder_list[0](
                    si_list[i], zi_list[i], inputs_type
                )
            else:
                # not shared
                # xi_fake = self.input_decoder_list[i](si_list[i], zi_list[i], inputs_type)
                # shared half
                mid_out = self.input_decoder_list[-1](
                    si_list[i], zi_list[i], inputs_type
                )
        ...
        for i in range(self.modality_num):
            for j in range(self.modality_num):
                ...
                if self.others["old"]:
                    ...
                    xi_fake = self.input_decoder_list[0](
                        si_list[i], zi_list[j], inputs_type
                    )
                else:
                    # not shared
                    ...
                    xi_fake = self.input_decoder_list[i](
                        si_list[i], mid_out, inputs_type
                    )
                xi_fake_list.append(xi_fake)
        return xi_fake_list
    ...
    def reconstruct_output_si(self, si_list):
        ...
        for i in range(self.modality_num):
            ...
            output = self.reconstruct_output_si_fused(
                [si_list[i]], torch.ones(bs, 1, device=self.device)
            )
    ...
    def compute_recon_loss_y_list(self, gt, y_list, mask, p=2):
        ...
        for i in range(len(y_list)):
            if mask[:, i].sum() == 0:
                continue
            idx += 1
            loss += (mask[:, i] * self.compute_recon_loss(gt, y_list[i], p)).sum() / (
                mask[:, i]
            ).sum()
        if idx == 0:
            return loss
    ...
    def compute_segmentation_loss_y(
        self, gt, y, weight=torch.tensor([1.0, 5.0, 5.0, 5.0])
    ):
        loss_seg = F.cross_entropy(
            y, gt.squeeze(1).long(), weight=weight.to(self.device)
        )
        loss_dice = 0
        y_act = F.softmax(y)
        for i in range(1, 4):
    ...
    def compute_segmentation_loss_y_list(
        self, gt, y_list, mask, weight=torch.tensor([1.0, 5.0, 5.0, 5.0])
    ):
        loss = torch.tensor(0.0, device=self.device)
        idx = 0
        for i in range(len(y_list)):
    ...
    def compute_recon_loss_x_list(self, gt_list, x_list, mask, p=2):
        ...
        for i in range(len(x_list)):
            ...
            idx += 1
            loss += (
                mask[:, i] * self.compute_recon_loss(gt_list[i], x_list[i], p)
            ).sum() / (mask[:, i]).sum()
        if idx == 0:
            return loss
        return loss / idx
        ...
        for i in range(mask.shape[1]):
            for j in range(mask.shape[1]):
                ...
                if mask_mix.sum() == 0:
                    continue
                loss += (
                    mask_mix * self.compute_recon_loss(gt_list[j], x_list[idx], p)
                ).sum() / mask_mix.sum()
                idx += 1
        if idx == 0:
            return loss
    ...
    def compute_kl_loss_standard(self, z_mean, z_log_var, mask):
        if mask.sum() == 0:
            loss = torch.tensor(0.0, device=self.device)
        kl = 0.5 * torch.sum(torch.exp(z_log_var) + z_mean ** 2 - 1.0 - z_log_var, 1)
    ...
    def compute_kl_loss_two_gaussian(
        self, z_mean, z_log_var, z_prior_mean, z_prior_log_var, mask
    ):
        if mask.sum() == 0:
            loss = torch.tensor(0.0, device=self.device)
        kl = 0.5 * (
            -1
            + (z_prior_log_var - z_log_var)
            + (torch.exp(z_log_var) + (z_mean - z_prior_mean) ** 2)
            / torch.exp(z_prior_log_var)
        )
        return (kl * mask.unsqueeze(1)).sum() / mask.sum()

    def compute_kl_loss_list_two_gaussian(
        self, z_mean_list, z_log_var_list, z_prior_mean_list, z_prior_log_var_list, mask
    ):
        loss = 0
        for i in range(len(z_mean_list)):
            loss += self.compute_kl_loss_two_gaussian(
                z_mean_list[i],
                z_log_var_list[i],
                z_prior_mean_list[i],
                z_prior_log_var_list[i],
                mask[:, i],
            )
    ...
    def compute_latent_z_loss(self, zi_mean_list, zi_mean_list_new, mask):
        ...
        for i in range(len(zi_mean_list)):
            ...
            loss += (
                mask[:, i].unsqueeze(1)
                * torch.abs(zi_mean_list[i] - zi_mean_list_new[i])
            ).sum() / mask[:, i].sum()
        if idx == 0:
            return loss
        return loss / idx
    ...
    def compute_similarity_s_loss(self, si_list, mask, margin=0.1):
        ...
        if mask_mix.sum() > 0:
            if self.s_sim_method == "cosine":
                ...
                loss = (
                    mask_mix
                    * torch.max(torch.zeros_like(mask[:, 0]), margin - sim + sim_mix)
                ).sum() / mask_mix.sum()
            else:
        ...
        for i in range(len(zi_list) - 1):
            ...
            for j in range(i + 1, len(zi_list)):
                ...
                cosine = self.compute_cosine(
                    zi, zj
                )  # diff modality, same subj, should be different
                cosine_mix = self.compute_cosine(
                    zi, zi_perm
                )  # same modality, diff subj, should be similar
                loss += (
                    mask_mix
                    * torch.max(
                        torch.zeros_like(mask[:, 0]), margin - cosine_mix + cosine
                    )
                ).sum() / mask_mix.sum()
        if idx == 0:
            return loss
    ...
    def compute_adversarial_loss(self, si_list, mask):
        ...
        if mask[:, i].sum() == 0:
            d_loss_0 = torch.tensor(0.0, device=self.device)
            g_loss_0 = torch.tensor(0.0, device=self.device)
        else:
            d_loss_0 = (
                mask[:, i]
                * F.binary_cross_entropy_with_logits(
                    d_0, torch.zeros_like(d_0), reduction="none"
                )
            ).sum() / mask[:, i].sum()
            g_loss_0 = (
                mask[:, i]
                * F.binary_cross_entropy_with_logits(
                    d_0, torch.ones_like(d_0), reduction="none"
                )
            ).sum() / mask[:, i].sum()
        if mask[:, j].sum() == 0:
            d_loss_1 = torch.tensor(0.0, device=self.device)
            g_loss_1 = torch.tensor(0.0, device=self.device)
        else:
            d_loss_1 = (
                mask[:, j]
                * F.binary_cross_entropy_with_logits(
                    d_1, torch.ones_like(d_1), reduction="none"
                )
            ).sum() / mask[:, j].sum()
            g_loss_1 = (
                mask[:, j]
