...
class MedicalDataset(Dataset):
    ...
    def __init__(
        self, data_path, task="reconstruction", contrast_idx=[0, 1, 2], transform=None
    ):
        # pdb.set_trace()
...
def load_checkpoint_by_key(values, checkpoint_dir, keys, device):
    ...
    if os.path.isfile(filename):
        ...
        print(
            "loaded checkpoint from '{}' (epoch: {}, monitor loss: {})".format(
                filename, epoch, checkpoint["monitor_loss"]
            )
        )
    else:
    ...
    for i in range(sample_num):
        ...
        if save_att_maps and len(alpha_keys) > 0:
            ...
            for k, key in enumerate(alpha_keys):
                ...
                att_map = skimage.transform.resize(
                    att_map, img_size_rot, preserve_range=True
                )
            ...
            background_tile = np.tile(
                background + background_intensity, (1, len(alpha_keys))
            )
            background_tile = np.concatenate(
                [background_tile, np.ones(background_tile.shape)], axis=0
            )
            background_hsv = skimage.color.rgb2hsv(
                np.dstack((background_tile, background_tile, background_tile))
            )
...
def save_test_result_by_volume(
    save_dict_list, test_dir, save_nifti=True, task="reconstruction", slice_per_subj=115
):
    # reorganize save_dict_list
    save_dict_new = {}
    for key in ["real_B", "fake_B"]:
        save_dict_new[key] = np.concatenate(
            [save_dict_list[i][key] for i in range(len(save_dict_list))], axis=0
        ).squeeze(axis=1)
    ...
    for i in range(subj_num):
        real_B = save_dict_new["real_B"][
            slice_per_subj * i : slice_per_subj * (i + 1), :, :
        ]
        fake_B = save_dict_new["fake_B"][
            slice_per_subj * i : slice_per_subj * (i + 1), :, :
        ]
...
def classification_metrics(real_B, fake_B):
    ...
    true_pos = ((fake_B == 1.0) & (real_B == 1.0)).sum()
    true_neg = ((fake_B == 0.0) & (real_B == 0.0)).sum()
    false_pos = ((fake_B == 1.0) & (real_B == 0.0)).sum()
    false_neg = ((fake_B == 0.0) & (real_B == 1.0)).sum()
...
def save_test_classification_result(label_list, prediction_list, save_path):
    ...
    prediction_list = np.array(
        [item for sublist in prediction_list for item in sublist]
    )
...
def majority_vote_volume_prediction(prediction_list, label_list, slice_per_subj=48):
    ...
    for subj in range(subj_num):
        data = prediction_list[
            subj * slice_per_subj + 10 : (subj + 1) * slice_per_subj - 10
        ]
        ...
        try:
            ...
            for contrast_name in self.contrast_list:
                ...
                if subj_id + "/" + contrast_name in self.data:
                    ...
                    imgs.append(
                        self.data[subj_id + "/" + contrast_name][
                            :,
                            :,
                            slice_idx
                            - self.block_size : slice_idx
                            + self.block_size
                            + 1,
                        ]
                    )
            ...
            if self.dataset_name == "ZeroDose":
                ...
                if subj_id + "/seg" in self.data.keys():
                    targets = self.data[subj_id + "/seg"][
                        :, :, slice_idx : slice_idx + 1
                    ]
                    targets[targets == 4] = 3.0
                else:
                    targets = np.zeros((self.image_size[0], self.image_size[1], 1))
            elif self.dataset_name == "Tau":
                if subj_id + "/pet_nifti/fulldose" in self.data.keys():
                    targets = self.data[subj_id + "/pet_nifti/fulldose"][
                        :, :, slice_idx : slice_idx + 1
                    ]
                else:
                    targets = np.zeros((self.image_size[0], self.image_size[1], 1))
            ...
            if self.dropoff and mask.sum() > 1:
                if np.random.rand() > 0.8:
                    ...
                    inputs[
                        ...
                        :,
                        drop_idx
                        * (2 * self.block_size + 1) : (drop_idx + 1)
                        * (2 * self.block_size + 1),
                    ] = 0
                    mask[drop_idx] = 0

            if self.skull_strip:
                brain_mask_input_slice = self.brain_mask[
                    :, :, slice_idx - self.block_size : slice_idx + self.block_size + 1
                ]
                brain_mask_input = np.tile(
                    brain_mask_input_slice, (1, 1, len(self.contrast_list))
                )
...
class TestDropoffDataset(Dataset):
    def __init__(
        self, data_path, fold, sel_idx_list, block_size=3, contrast_list=["T1"]
    ):
        self.data = h5py.File(os.path.join(data_path, "ZeroDose_FDG_All.h5"), "r")
        self.subj_list, self.idx_list = self.load_idx_list(
            os.path.join(data_path, "fold" + str(fold) + "_test.txt")
        )
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            inputs = np.concatenate(
                [inputs, np.zeros((3, inputs.shape[1], inputs.shape[2]))], 0
            )  # (157,189) -> (160,192)
            inputs = np.concatenate(
                [inputs, np.zeros((inputs.shape[0], 3, inputs.shape[2]))], 1
            )
            inputs = np.transpose(inputs, (2, 0, 1))
            # inputs = np.nan_to_num(inputs, nan=0.)
            targets = self.data[subj_id + "/PET"][:, :, slice_idx : slice_idx + 1]
            targets = np.concatenate(
                [targets, np.zeros((3, targets.shape[1], targets.shape[2]))], 0
            )
            targets = np.concatenate(
                [targets, np.zeros((targets.shape[0], 3, targets.shape[2]))], 1
            )
            targets = np.transpose(targets, (2, 0, 1))
...
class ZeroDoseDataAll(object):
    def __init__(
        ...
        if dataset_name == "ZeroDose":
            ...
            if norm_type == "mean":
                data = h5py.File(
                    os.path.join(data_path, "ZeroDose_FDG_All_1103.h5"), "r"
                )
            else:
                data = h5py.File(
                    os.path.join(data_path, "ZeroDose_FDG_All_1103_zscore_10.h5"), "r"
                )
            ...
            if len(contrast_list) == 2:
                subj_list_train, idx_list_train = self.load_idx_list(
                    os.path.join(data_path, "fold" + str(fold) + "_train_1103_sel.txt")
                )
                subj_list_val, idx_list_val = self.load_idx_list(
                    os.path.join(data_path, "fold" + str(fold) + "_val_1103_sel.txt")
                )
                subj_list_test, idx_list_test = self.load_idx_list(
                    os.path.join(data_path, "fold" + str(fold) + "_test_1103_sel.txt")
                )
            elif len(contrast_list) == 3:
                subj_list_train, idx_list_train = self.load_idx_list(
                    os.path.join(
                        data_path, "fold" + str(fold) + "_train_3contrasts_sel.txt"
                    )
                )
                subj_list_val, idx_list_val = self.load_idx_list(
                    os.path.join(
                        data_path, "fold" + str(fold) + "_val_3contrasts_sel.txt"
                    )
                )
                subj_list_test, idx_list_test = self.load_idx_list(
                    os.path.join(
                        data_path, "fold" + str(fold) + "_test_3contrasts_sel.txt"
                    )
                )
                ...
                subj_list_train, idx_list_train = self.load_idx_list(
                    os.path.join(
                        data_path, "fold" + str(fold) + "_train_4contrasts_sel_all.txt"
                    )
                )
                subj_list_val, idx_list_val = self.load_idx_list(
                    os.path.join(
                        data_path, "fold" + str(fold) + "_val_4contrasts_sel_all.txt"
                    )
                )
                subj_list_test, idx_list_test = self.load_idx_list(
                    os.path.join(
                        data_path, "fold" + str(fold) + "_test_4contrasts_sel_all.txt"
                    )
                )
            ...
            subj_list_train, idx_list_train = self.load_idx_list(
                os.path.join(data_path, "fold_BraTS_" + str(fold) + "_train_noval.txt")
            )
            subj_list_val, idx_list_val = self.load_idx_list(
                os.path.join(data_path, "fold_BraTS_" + str(fold) + "_val_noval.txt")
            )
            subj_list_test, idx_list_test = self.load_idx_list(
                os.path.join(data_path, "fold_BraTS_" + str(fold) + "_test_noval.txt")
            )
            ...
            if norm_type == "mean":
                ...
                data = h5py.File(
                    os.path.join(data_path, "NCANDA_All_zscore_10.h5"), "r"
                )
            subj_list_train, idx_list_train = self.load_idx_list(
                os.path.join(data_path, "fold_NCANDA_" + str(fold) + "_train.txt")
            )
            subj_list_val, idx_list_val = self.load_idx_list(
                os.path.join(data_path, "fold_NCANDA_" + str(fold) + "_val.txt")
            )
            subj_list_test, idx_list_test = self.load_idx_list(
                os.path.join(data_path, "fold_NCANDA_" + str(fold) + "_test.txt")
            )
            ...
            subj_list_train, idx_list_train = self.load_idx_list(
                os.path.join(data_path, "fold_Tau_" + str(fold) + "_train.txt")
            )
            subj_list_val, idx_list_val = self.load_idx_list(
                os.path.join(data_path, "fold_Tau_" + str(fold) + "_val.txt")
            )
            subj_list_test, idx_list_test = self.load_idx_list(
                os.path.join(data_path, "fold_Tau_" + str(fold) + "_test.txt")
            )

        brain_mask_nib = nib.load(os.path.join(data_path, "tpm_mask.nii"))
        brain_mask = brain_mask_nib.get_fdata()

        train_dataset = nc.SafeDataset(
            ZeroDoseDataset(
                dataset_name,
                data,
                subj_list_train,
        ...
        try:
            ...
            for contrast_name in self.contrast_list:
                ...
                if subj_id + "/" + contrast_name in self.data:
                    if self.dataset_name == "ZeroDose":
                        imgs.append(
                            self.data[subj_id + "/" + contrast_name][:, :, 45:-47]
                        )
                    else:
                        imgs.append(
                            self.data[subj_id + "/" + contrast_name][:, :, 45:-46]
                        )
                    mask.append(1)
                else:
                    imgs.append(
    ...
    def __init__(
        ...
        if dataset_name == "BraTS":
            if norm_type == "mean":
                ...
                data = h5py.File(os.path.join(data_path, "BraTS_All_zscore_10.h5"), "r")
            subj_list_train = self.load_idx_list(
                os.path.join(
                    data_path, "fold_BraTS_3d_" + str(fold) + "_train_noval.txt"
                )
            )
            subj_list_val = self.load_idx_list(
                os.path.join(data_path, "fold_BraTS_3d_" + str(fold) + "_val_noval.txt")
            )
            subj_list_test = self.load_idx_list(
                os.path.join(
                    data_path, "fold_BraTS_3d_" + str(fold) + "_test_noval.txt"
                )
            )
        elif dataset_name == "ZeroDose":
            if norm_type == "mean":
                data = h5py.File(
                    os.path.join(data_path, "ZeroDose_FDG_All_1103_norm.h5"), "r"
                )
            else:
                data = h5py.File(
                    os.path.join(data_path, "ZeroDose_FDG_All_1103_zscore_10.h5"), "r"
            ...
            subj_list_train = self.load_idx_list(
                os.path.join(data_path, "ZeroDose_3d_all.txt")
            )
            subj_list_val = self.load_idx_list(
                os.path.join(data_path, "ZeroDose_3d_all.txt")
            )
        ...
        train_dataset = ZeroDoseDataset3D(
            dataset_name,
            data,
            subj_list_train,
            contrast_list=contrast_list,
        ...
        test_dataset = ZeroDoseDataset3D(
            dataset_name,
            data,
            subj_list_test,
            contrast_list=contrast_list,
...
# load model
def load_checkpoint_by_key(
    values, checkpoint_dir, keys, device, ckpt_name="model_best.pth.tar"
):
    """
    the key can be state_dict for both optimizer or model,
    value is the optimizer or model that define outside
    ...
    if os.path.isfile(filename):
        ...
        print(
            "loaded checkpoint from '{}' (epoch: {}, monitor metric: {})".format(
                filename, epoch, checkpoint["monitor_metric"]
            )
        )
    else:
        raise ValueError("No correct checkpoint")
...
def save_config_yaml(ckpt_path, config):
    ...
    for key in config.keys():
        if (
            ...
            or isinstance(config[key], float)
            or isinstance(config[key], str)
            or isinstance(config[key], list)
            or isinstance(config[key], dict)
        ):
            continue
...
def compute_reconstruction_metrics_single(target, pred):
    ...
    try:
        ...
        psnr_pred = skimage.metrics.peak_signal_noise_ratio(
            target, pred, data_range=range
        )
    except:
        pdb.set_trace()
        psnr_pred = float("nan")
    try:
        ...
        ssim_pred = skimage.metrics.structural_similarity(
            target, pred, data_range=range
        )
    except:
        ssim_pred = float("nan")
...
def compute_segmentation_metrics_single(target, pred):
    ...
    for i in range(3):
        ...
        dice = (2.0 * intersection.sum() + 1) / (
            (target == i + 1).sum() + (pred[i] > 0.5).sum() + 1
        )
