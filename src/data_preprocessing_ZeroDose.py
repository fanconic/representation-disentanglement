...
brain_mask_nib = nib.load(
    os.path.join("/data/jiahong/project_zerodose_pytorch/data/", "tpm_mask_new.nii")
)
...
for i, subj_id in enumerate(subj_all_dict.keys()):
    ...
    for contrast_name in subj_dict.keys():
        ...
        if (
            img.shape != (157, 189, 156)
            or np.nanmax(img) == 0
            or np.isnan(img[:, :, 20:-20]).sum() > 100000
        ):
            print(subj_id)
...
def select_complete_subj(f, subj_id_list):
    ...
    for subj_id in subj_id_list:
        ...
        if (
            "T1" in f[subj_id].keys()
            and "T1c" in f[subj_id].keys()
            and "T2_FLAIR" in f[subj_id].keys()
            and "ASL" in f[subj_id].keys()
            and "PET" in f[subj_id].keys()
        ):
...
for fold in range(5):
    subj_id_list_test = subj_id_list_sel[
        fold * int(0.2 * num_subj) : (fold + 1) * int(0.2 * num_subj)
    ]
    subj_id_list_train_val = (
        subj_id_list_sel[: fold * int(0.2 * num_subj)]
        + subj_id_list_sel[(fold + 1) * int(0.2 * num_subj) :]
    )
    ...
    save_data_txt(
        "../data/fold" + str(fold) + "_train_4contrasts_sel_all.txt", subj_id_list_train
    )
    save_data_txt(
        "../data/fold" + str(fold) + "_val_4contrasts_sel_all.txt", subj_id_list_val
    )
    save_data_txt(
        "../data/fold" + str(fold) + "_test_4contrasts_sel_all.txt", subj_id_list_test
    )
